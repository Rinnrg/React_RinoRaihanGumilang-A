    ...
    <div className="flex gap-4">
      {options.map((option, index) => (
        ...
        >
          ...
          />
          <span
            className={`w-4 h-4 rounded-full flex items-center justify-center border ${selected === option.value ? 'bg-[#69BE28] border-[#69BE28]' : 'border-[#656666]'}`}
            style={{ width: '16px', height: '16px', borderWidth: '1px' }}
          >
            {selected === option.value && <span className="w-2.5 h-2.5 bg-[#69BE28] rounded-full"></span>}
          </span>
